        ...
        for (int r = 0; r < 600; r++) {
            ...
            */
            ...
            for (size_t i = 0; i < 100; i++) {
                ...
                total_step_time += std::chrono::duration_cast<std::chrono::microseconds>(end-start).count();
            }

            epoch_time += total_step_time;
            il.update_weights(.002);
            auto end_mb = std::chrono::high_resolution_clock::now();
            auto mini_batch_time = std::chrono::duration_cast<std::chrono::microseconds>(end_mb-start_mb).count();
            std::cout<< "Time per mini-batch: " << mini_batch_time << std::endl;
		    std::cout<< "Avg Time Per Step: " << total_step_time/(100) << std::endl;
        }
        myfile.close();
    }
